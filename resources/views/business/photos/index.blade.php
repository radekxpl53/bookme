...
<div class="container py-4">
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-header bg-white">
            <h5 class="mb-0">Dodaj nowe zdjęcia</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('biznes.lokale.zdjecia.store', $business) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h5 class="mb-0">Obecne portfolio lokalu</h5>
        </div>
        <div class="card-body">
            @if($photos->isEmpty())
                <p class="text-muted mb-0">Nie wgrano jeszcze żadnych zdjęć.</p>
            @else
                <div class="row g-3">
                    @foreach($photos as $photo)
                        <div class="col-6 col-md-4 col-lg-3">
                            <div class="card h-100">
                                <img src="{{ asset('storage/' . $photo->path) }}" class="card-img-top" alt="Zdjęcie salonu" style="object-fit: cover; height: 200px;">
                                <div class="card-footer bg-white text-center">
                                    <form action="{{ route('biznes.lokale.zdjecia.destroy', [$business, $photo]) }}" method="POST" onsubmit="return confirm('Na pewno chcesz usunąć to zdjęcie?');">
                                        @csrf
                                        @method('DELETE')
